...
#include <vector>
#include <set>
#include "arco_pesato.hpp"

class creazione_grafo{
	
   std::set<arco_pesato> edges;
...
void add_edge(const arco_pesato& a_edge){
	edges.insert(a_edge); 
};
...
std::vector<arco_pesato> all_edges() const{
	std::vector<arco_pesato> archi;
	for(const auto& edge : edges){
...
int edge_number(const arco_pesato& e) const{
	int i = 0;
	for (const auto& edge : edges){
		if (edge == e){
			return i;
...
// DA MANDARE A CRISTINEL 
int find_index(int nodo_partenza, int nodo_arrivo) const{
	int i=0;
	int u=(nodo_arrivo<= nodo_partenza) ? nodo_arrivo : nodo_partenza; //nodo piu piccolo quindi from
	v=(nodo_arrivo >nodo_partenza) ? nodo_arrivo : nodo_partenza;  //nodo to
	for (const auto& edge : edges){
...
arco_pesato edge_at(size_t j) const{
    size_t k=0;
	for (const auto& edge: edges){
		if(k == j){
			return edge;
