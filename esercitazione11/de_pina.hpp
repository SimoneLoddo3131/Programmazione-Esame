...
std::vector<bool> find_minimal_cycle(int num_nodi, int num_archi, std::vector<arco_pesato>& archi, std::vector<bool>& Si){
	int best_size = std::numeric_limits<int>::max();
	std::vector<bool> best_cycle(num_archi, false);
	
	for (int w = 0; w < num_nodi; w++){
		std::vector<std::vector<lifting_edge>> lifting_graph (2 * (num_nodi+1));
...
std::vector<std::vector<bool>> de_pina(int num_nodi, int num_archi, std::vector<arco_pesato>& archi, std::vector<std::vector<bool>>& S, int k){
