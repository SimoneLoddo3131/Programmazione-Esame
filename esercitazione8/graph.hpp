...
class Unidirected_graph{
	...
	std::set<Unidirected_edge<T>> neighbours(T n){
		std::set<Unidirected_edge<T>> archi_vicini;
		for(const auto& arco : archi){
        if (arco.from() == n  ||  arco.to()== n) {//controllato come si puo aggiungere con insert
        archi_vicini.insert(arco);
            }	
	}
	return archi_vicini;
} 
